...
class Sinmido_Booking_REST {
	...
	public function create_reservation( WP_REST_Request $request ) {
		...
		$sys = $this->get_system_settings_array();
		if ( $sys['inquiry_mode'] === 'enabled' ) {
			$inquiry_msg = __( '現在、予約を受け付けておりません。お問い合わせください。', 'sinmido-booking' );
			if ( ! empty( $sys['inquiry_phone'] ) ) {
				$inquiry_msg .= ' ' . sprintf( __( 'お問い合わせ先：%s', 'sinmido-booking' ), $sys['inquiry_phone'] );
			}
			return new WP_Error( 'inquiry_only', $inquiry_msg, array( 'status' => 403 ) );
		}
		...
		foreach ( $blacklist as $row ) {
			...
			if ( $bl_email !== '' && $email_n === $bl_email ) {
				return new WP_Error( 'blocked', __( 'このメールアドレスでは、当サイトから予約・メール送信を行うことができません。', 'sinmido-booking' ), array( 'status' => 403 ) );
			}
			if ( $bl_phone !== '' && $phone_n !== '' && $phone_n === $bl_phone ) {
				return new WP_Error( 'blocked', __( 'この電話番号では、当サイトから予約・メール送信を行うことができません。', 'sinmido-booking' ), array( 'status' => 403 ) );
			}
		}
		...
		update_post_meta( $reservation_id, '_sb_status', 'tentative' );
	...
	private function send_booking_emails( $reservation_id, $event_post, $name, $email, $phone, $custom, $confirmed_date, $confirmed_time_start, $confirmed_time_end ) {
		...
		if ( ! empty( $sys['notify_on_booking'] ) && ! empty( $sys['admin_recipient'] ) ) {
			...
			$sender  = ! empty( $sys['admin_sender'] ) ? $sys['admin_sender'] : $default_from;
			$headers = array(
		...
		if ( ! empty( $sys['send_confirm_email'] ) && $email !== '' ) {
			...
			$subject = $this->replace_mail_placeholders( $subject, $replace );
			$body    = $this->replace_mail_placeholders( $body, $replace );
			$sender  = ! empty( $sys['customer_sender'] ) ? $sys['customer_sender'] : $default_from;
			$headers = array(
				'Content-Type: ' . ( $is_html ? 'text/html' : 'text/plain' ) . '; charset=UTF-8',
				'From: ' . $sender,
			);
